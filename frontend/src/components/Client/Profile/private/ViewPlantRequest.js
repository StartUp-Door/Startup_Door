...
const ProblemList = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/service/plant123/ongoing123');
...
const handleDelete = async (e, id) => {
  e.stopPropagation();
  try {
    const response = await axios.delete(`/client/deletePlantRequest/${id}`);
  ...
  return (   
    ...
    <div>
      <h3 style={{textAlign: 'center', marginTop:20}}>_Requested Jobs for a Plants & Crops_</h3>
              ...
              return (
                <tr key={index+1}>
                  ...
                  <td style={{width:40, textAlign: 'center'}}>
                    <button
                      onClick={(e) => handleDelete(e, restaurant.jobplant_id)}
