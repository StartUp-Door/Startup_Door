...
const ProblemList = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/service/technician/ongoing/2');
  ...
  const accept = async (e, id) => {
    e.preventDefault();
    try {
      const response = await axios.put(`/client/requestTechPut/${id}`);
    } 
    catch (err) {
      console.log(err);
    }
  ...
  const decline = async (e, id) => {
    e.preventDefault();
    try {
      const response = await axios.put(`/client/requestTechDonotPut/${id}`);
    } 
    catch (err) {
      console.log(err);
    }
  ...
  return (   
    ...
    <div>
      <h3 style={{textAlign: 'center', marginTop:20}}>Technician Ongoing Works</h3>
            ...
            allproblems && allproblems.map((restaurant,index) => {
              return (
                <tr key={index+1}>
                  ...
                  <td style={{width:80, textAlign: 'center', display: 'flex', backgroundColor: 'white'}}>
                    <form onSubmit={(e) => accept(e, restaurant.jobtech_id)}>
                      <button
                    ...
                    <form onSubmit={(e) => decline(e, restaurant.jobtech_id)}>
                      <button
                        style={{width:20}}
