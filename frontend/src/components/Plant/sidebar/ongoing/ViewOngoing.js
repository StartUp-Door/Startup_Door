...
const ProblemList = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/service/plant/ongoing/2');
  ...
  const accept = async (e, id) => {
    e.preventDefault();
    try {
      const response = await axios.put(`/client/requestPlantPut/${id}`);
    } 
    catch (err) {
      console.log(err);
    }
  ...
  const decline = async (e, id) => {
    e.preventDefault();
    try {
      const response = await axios.put(`/client/requestPlantDonotPut/${id}`);
    } 
    catch (err) {
      console.log(err);
    }
  ...
  return (   
    ...
    <div>
      <h3 style={{textAlign: 'center', marginTop:20}}>Plants & Crops Ongoing Works</h3>
        <Table striped bordered hover condensed pagination insertRow deleteRow search variant="" style={{marginLeft:10, marginRight: '-10px', marginTop:10, boxShadow: '0px 0px 16px -8px rgba(0, 0, 0, 0.68)'}}>
        <thead>
            ...
            allproblems && allproblems.map((restaurant,index) => {
              return (
                <tr key={index+1}>
                  ...
                  <td style={{width:77, textAlign: 'center', display: 'flex'}}>
                    <form onSubmit={(e) => accept(e, restaurant.jobplant_id)}>
                      <button
                    ...
                    <form onSubmit={(e) => decline(e, restaurant.jobplant_id)}>
                      <button
                        style={{width:20}}
